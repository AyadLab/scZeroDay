...
"Plasma Cell Myeloma" %in% unique(meta.filt$OncotreeSubtype)

################################################################################
# MM

# make vector of 0/1 indicating which cell lines are Glioblastoma
gbm.ind.var <- as.numeric(meta.filt$OncotreeSubtype == "Plasma Cell Myeloma")
# 20 cell lines
...
gbm.meta <- meta.filt[which(meta.filt$OncotreeSubtype == "Plasma Cell Myeloma"), ]
# write.table(
#   gbm.meta,
...
gbm <- subset(
  meta.filt,
  meta.filt$OncotreeSubtype == "Plasma Cell Myeloma"
)
# subset effects data
gbm.efx <- subset(
  efx,
...
lower <- gbm.stats_m[which(gbm.stats_m$EffectSize < 0), ] # all lower 7860
sig.lower <- lower[which(lower$q.value < 5e-2), ] # all sig lower 1036
killing <- sig.lower[which(sig.lower$mEffectScore <= -0.5), ] # all sig lower w/ killing effect 274
...
write.table(
  killing,
  file = "Output/Rdata/05_MM_essential.genes_killing_2024.01.31.csv",
  quote = FALSE, row.names = TRUE, col.names = TRUE, sep = ","
)
