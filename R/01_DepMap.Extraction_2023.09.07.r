...
source("r.input_run_lm_stats_limma_FUNCTION_2023.03.23.R")
source("r.input_run_lm_fit_only_2023.05.31.R")
...
efx <- read.delim(
  file = "r.input_CRISPRGeneEffect.csv", 
  sep = ",", 
  row.names = 1, 
  check.names = FALSE
)
...
meta <- read.delim(
  file = "r.input_metadata_Model.csv", 
  sep = ",", 
  row.names = 1
)
